...
if [[ "$1" == apache2* ]] || [ "$1" == php-fpm ]; then
	...
	if ! [ -e wordpress/index.php -a -e wordpress/wp-includes/version.php ]; then
...
EOPHP
			chown "$user:$group" wordpress/wp-config.php
		fi
		...
		set_config() {
			...
			sed -ri -e "s/($start\s*).*($end)$/\1$(sed_escape_rhs "$(php_escape "$value" "$var_type")")\3/" wordpress/wp-config.php
		}
		...
		for unique in "${uniqueEnvs[@]}"; do
			...
			if [ -n "${!uniqVar}" ]; then
				...
				currentVal="$(sed -rn -e "s/define\((([\'\"])$unique\2\s*,\s*)(['\"])(.*)\3\);/\4/p" wordpress/wp-config.php)"
